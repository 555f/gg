...
func GenConfig(c options.Config) func(f *file.GoFile) {
	return func(f *file.GoFile) {
		...
		f.Func().Id(c.ConstructName).Params().Params(Id("c").Op("*").Do(stType), Id("errs").Index().Error()).BlockFunc(func(g *Group) {
			...
			walkFields(c.Fields, func(parent *options.ConfigField, field options.ConfigField) {
				...
				lookupIf := g.If(List(Id("s"), Id("ok")).Op(":=").Qual("os", "LookupEnv").
					...
					BlockFunc(func(g *Group) {
						if isString {
							...
							g.Do(gen.CheckErr(
								Id("errs").Op("=").Append(Id("errs"), Qual("fmt", "Errorf").Call(Lit("env "+envName+" failed parse: %w"), Err())),
							))
						...
						if !field.UseZero && field.Required {
							if field.Zero != "" {
								g.If(Add(code).Dot(field.FieldName).Op("==").Id(field.Zero)).Block(
									Id("errs").Op("=").Append(Id("errs"), Qual("errors", "New").Call(Lit("env "+envName+" empty"))),
								)
							}
						}
					})
				if field.Required {
					lookupIf.Else().Block(
						Id("errs").Op("=").Append(Id("errs"), Qual("errors", "New").Call(Lit("env "+envName+" not set"))),
					)
				}
			})
