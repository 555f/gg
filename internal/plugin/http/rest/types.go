...
func GenTypes() func(f *file.GoFile) {
	return func(f *file.GoFile) {
		...
		f.Func().Id("pathParamsToContext").Params(
			...
			Return(Qual("context", "WithValue").Call(Id("ctx"), Id("pathParamsContextKey"), Id("pathParams"))),
		)

		f.Func().Id("pathParamsFromContext").Params(
			Id("ctx").Qual("context", "Context"),
		).Params(Id("result").Qual("net/url", "Values")).Block(
			List(Id("result"), Id("_")).Op("=").Id("ctx").Dot("Value").Call(Id("pathParamsContextKey")).Assert(Qual("net/url", "Values")),
			Return(),
		)
