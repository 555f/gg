...
func (m *profileControllerBaseMiddleware) Create(ctx context.Context, token string, firstName string, lastName string, address string, old int, age time.Time, sleep time.Duration) (profile *dto.Profile, err error) {
	defer func() {
		if s, ok := m.mediator.(profileControllerCreateBaseMiddleware); ok {
			s.Create(ctx, token, firstName, lastName, address, old, age, sleep)
		}
	}()
	return m.next.Create(ctx, token, firstName, lastName, address, old, age, sleep)
}
...
type profileControllerCreateBaseMiddleware interface {
	Create(ctx context.Context, token string, firstName string, lastName string, address string, old int, age time.Time, sleep time.Duration)
}
