...
import (
	middleware "github.com/555f/gg/examples/rest-service-echo/internal/middleware"
...
func (s *ProfileControllerLoggingMiddleware) Create(firstName string, lastName string, address string, zip int) (profile *dto.Profile, err error) {
	defer func(now time.Time) {
		logger := log.WithPrefix(s.logger, "message", "call method - Create", "firstName", "firstName", firstName, "lastName", "lastName", lastName, "address", "address", address, "zip", "zip", zip)
		if err != nil {
...
func (s *ProfileControllerLoggingMiddleware) DownloadFile(id string, onlyCloud bool) (data string, err error) {
	defer func(now time.Time) {
		logger := log.WithPrefix(s.logger, "message", "call method - DownloadFile", "id", "id", id, "onlyCloud", "onlyCloud", onlyCloud)
		if err != nil {
			if e, ok := err.(errLevel); ok {
				logger = levelLogger(e, logger)
			} else {
	...
	data, err = s.next.DownloadFile(id, onlyCloud)
	return
}
