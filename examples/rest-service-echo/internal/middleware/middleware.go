...
func (m *profileControllerBaseMiddleware) DownloadFile(id string, onlyCloud bool) (data string, err error) {
	defer func() {
		if s, ok := m.mediator.(profileControllerDownloadFileBaseMiddleware); ok {
			s.DownloadFile(id, onlyCloud)
		}
	}()
	return m.next.DownloadFile(id, onlyCloud)
}
...
type profileControllerDownloadFileBaseMiddleware interface {
	DownloadFile(id string, onlyCloud bool)
}
